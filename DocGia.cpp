...
int getMaTheTuFile(const char* filename) {
    ...
    if (!fin) {
        cout << "Khong the mo file ma the!" << endl;
        return -1;
    }
    string maTheStr;
    getline(fin, maTheStr);
    if (maTheStr.empty()) {
        cout << "Het ma the de cap!" << endl;
        fin.close();
        return -1;
    }
    ...
    if (!fout) {
        cout << "Khong the ghi vao file ma the" << endl;
        return -1;
    }
    fout << cacMaTheConLai;
    fout.close();
...
void save_File(TREE_DOCGIA root, const char* filename){
    ...
    while(!q.empty()){
        TREE_DOCGIA node = q.getFront(); q.pop();
...
void read_File(TREE_DOCGIA &root, const char* filename) {
    ifstream fin(filename);
    if (!fin.is_open()) {
        cout<<"Khong mo duoc file danh sach doc gia!"<<endl;
        return;
    }
...
void draw_GiaoDienXoaDocGia(){
    clrscr();
    SetColor(14);
    CreateBoxDouble(50, 1, "   XOA DOC GIA   ", 17);
    cout<< setfill(' ');
    SetColor(7);
...
bool Enter_Xoa(TREE_DOCGIA &root, TREE_DOCGIA &p){
    ...
    gotoxy(84, 12); cout << "Ban co chac muon xoa doc gia nay? (y/n): ";
    char confirmChar = GetYesNoInput(84 + string("Ban co chac muon xoa doc gia nay? (y/n): ").length(), 12);
    if (confirmChar == 'y') {
        Delete_DocGia(root, stoi(maThe));
        save_File(root, "txt\\DanhSachDocGia.txt");
        ShowCur(false);
        XoaKhuVucTraSach();
        return true;
...
void XoaDocGia(TREE_DOCGIA &root) {
    ...
    while(true){
        ...
        if(key == 224){
            ...
            ThongBao("Thoat khoi chuc nang xoa doc gia");
            break;
        }
    }
...
void draw_GiaoDienSuaDocGia(){
    clrscr();
    SetColor(14);
    CreateBoxDouble(50, 1, "   SUA DOC GIA   ", 17);
    cout<< setfill(' ');
    SetColor(7);
...
void SuaDocGia(TREE_DOCGIA &root){
    ...
    TREE_DOCGIA p;
    string tmpHo, tmpTen, tmpGioiTinh;
    int tmpTrangThai;
    ...
    while(true){
        ...
        if(key == 224){
            ...
            p = nullptr;
            if(Enter_SuaDocGia(root, p)){
                tmpHo = p->data.ho;
                tmpTen = p->data.ten;
                tmpGioiTinh = p->data.gioitinh;
                tmpTrangThai = p->data.trangThai;
                currentField = HO;
                int formX = 90, formY = 15;
                ...
                while(isEditing){
                    ...
                    if(key == 0 || key == 224){
                        key = _getch();
                        switch(key){
                            case 72: currentField = (currentField > HO) ? currentField - 1 : TRANGTHAI; break;
                            case 80: currentField = (currentField < TRANGTHAI) ? currentField + 1 : HO; break;
                            case 60:{
                                ShowCur(true);
            ...
            ThongBao("Thoat khoi chuc nang sua doc gia");
            break;
        }
    }
    delete[] Array;
...
void print_DsDangMuon(TREE_DOCGIA root, DS_DauSach &ds) {
    ...
    while(true){
        ...
        HienThiDanhSachDocGia(tempDocGiaArray, currentPage_DG, totalPages_DG, totalNode_DG);
        ...
        bool isViewing = true;
        while(isViewing){
            HienThiDanhSachMuon(tmpMuonArray, ds, currentMuonPage, totalMuonPages, totalMuonNode, 84, 11);
            key = _getch();
            if(key == 0 || key == 224){
                key = _getch();
